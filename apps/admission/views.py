...
from apps.payement.models import Payement
...
def new_registration(request):
    ...
    except ActiveAcademinYear.DoesNotExist:
        active_year = None
...
def registration_detail(request, registration_id):
    ...
    return render(request, 'admission/registration-detail.html',
...
def edit_registration(request, registration_id):
    ...
    return render(request, 'admission/edit-registration.html',
...
def delete_registration(request, pk):
    ...
    if request.method == 'POST':
        ...
        data['html_form'] = render_to_string('admission/partial-registration-delete.html',
            context, request=request)
    return JsonResponse(data)


# implement inscription list
@login_required
def inscription(request):
    # Looking for Admitted student only
    inscription = Inscription.objects.all()
    adprocess = AdmissionProcess.objects.filter(commitee_decision='approuvé')
    payements = Payement.objects.all()
    context =  {'adprocess': adprocess, 'payement': payements, 'inscription': inscription}
...
def new_admission_process(request, id):
    ...
    except Registration.DoestNotExists:
        registration = None
    ...
    if request.method == 'POST':
        ...
        form = AdmissionProcessForm(initial={'registree_name':registree_name,
            'registree_number':registree_number,'registree': registree})
...
def new_inscription(request, id):
    try:
       active_year = ActiveAcademicYear.objects.last().academic_year.id
        ...
        active_year = None
    registration = AdmissionProcess.objects.get(id=id)
